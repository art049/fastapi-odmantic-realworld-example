...
async def get_article_comments(
    ...
    return MultipleCommentsResponse.from_article_instance(article)
...
async def add_article_comment(
    ...
    article = await get_article_by_slug(engine, slug)
    comment_instance = CommentModel(author=user_instance, **new_comment.dict())
    await add_new_comment(engine, article, comment_instance)
    return SingleCommentResponse(comment=comment_instance)
